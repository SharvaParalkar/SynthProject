...
void setup() {
  Serial.begin(115200);
  // Configure button matrix row
  pinMode(rowPin, OUTPUT);
  digitalWrite(rowPin, LOW);
  // Configure button inputs (columns)
  ...
  cfg.pin_bck = 6;       // Bit Clock (BCK)
  cfg.pin_ws = 7;        // Word Select / LRCK
  cfg.pin_data = 5;      // Data pin (DIN)
...
void play(float *output) {
  ...
  if (buttonState4 == LOW) {
    // Play full Fmaj7 chord using individual oscillators
    double chord = (
      oscOne.sawn(174.61) +     // F
      oscTwo.sawn(220.00) +     // A
      oscThree.sawn(261.63) +   // C
      oscFour.sawn(329.63)      // E
    ) / 4.0;
    double filtered = filter.lores(chord, 1000, 0.8);
    output[0] = filtered * 0.3;
    output[1] = output[0];
